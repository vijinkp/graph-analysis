...
def get_graphs(year, folder_path, master_map):
	return ([get_igraph(join(folder_path,file), master_map) for file in listdir(folder_path) if '.net' in file] , ['{0}_{1}'.format(year, file.split('.')[0]) for file in listdir(folder_path) if '.net' in file])
...
if __name__ == '__main__':
	...
	for yr1, yr2 in year_tup:
		yr1_graphs , yr1_labels = get_graphs(yr1, join(root_folder, str(yr1), 'communities'), master_map)
		yr2_graphs , yr2_labels = get_graphs(yr2, join(root_folder, str(yr2), 'communities'), master_map)
		...
		K_WL_subset = K_WL[:yr1_graphs.size(), yr1_graphs.size():]
